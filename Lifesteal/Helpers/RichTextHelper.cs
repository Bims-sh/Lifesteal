...
public class RichTextHelper
{
    ...
    /// <summary>
    /// Thanks Rain! ;)
    /// Credits to https://github.com/RainOrigami/
    /// From https://modules.battlebit.community/module/1
    /// </summary>

    public static string NewLine() => "<br>";

    public static string Color(string? color = null)
    {
        return color is null ? Colors.White : $"<{color}>";
    }

    public static string FromColorName(string colorName)
    {
        FieldInfo? color = typeof(Colors).GetFields().FirstOrDefault(x => x.Name.ToLower() == colorName.ToLower());
        if (color != null) return color.GetValue(null)!.ToString()!;
        Program.Logger.Error($"No color found with name {colorName}");
        return "<#FFFFFF>";

    }

    public static string Align(string? alignment = null)
    {
        return alignment is null ? Alignments.None : $"<align=\"{alignment}\">";
    }

    public static string FromAlignmentName(string alignmentName)
    {
        FieldInfo? alignment = typeof(Alignments).GetFields().FirstOrDefault(x => x.Name.ToLower() == alignmentName.ToLower());
        if (alignment != null) return alignment.GetValue(null)!.ToString()!;
        Program.Logger.Error($"No alignment found with name {alignmentName}");
        return Alignments.None;

    }

    public static string Alpha(string? alpha = null)
    {
        return alpha is null ? "<alpha=#FF>" : $"<alpha={alpha}>";
    }

    public static string CharacterSpacing(int pixels) => $"<cspace={pixels}>";
    public static string CharacterSpacing(float em) => $"<cspace={em}em>";

    public static string Font(string? fontName)
    {
        return fontName is not null ? $"<font=\"{fontName}\">" : "<font=\"default\">";
    }

    public static string Indent(int? percentage = null)
    {
        return percentage is null ? "</indent>" : $"<indent={percentage}%>";
    }

    public static string LineHeight(int percentage) => $"<line-height={percentage}%>";

    public static string LineIndentation(int percentage) => $"<line-indent={percentage}%>";

    public static string Lowercase(bool lowercase) => lowercase ? "<lowercase>" : "</lowercase>";
    public static string Uppercase(bool uppercase) => uppercase ? "<uppercase>" : "</uppercase>";
    public static string Smallcaps(bool smallcaps) => smallcaps ? "<smallcaps>" : "</smallcaps>";

    public static string Margin(int pixels) => $"<margin={pixels}>";
    public static string Margin(float em) => $"<margin={em}em>";

    public static string Monospacing(int pixels) => $"<mspace={pixels}>";
    public static string Monospacing(float em) => $"<mspace={em}em>";
    public static string Monospacing() => "</mspace>";

    public static string Noparse(bool noparse) => noparse ? "<noparse>" : "</noparse>";

    public static string NonBreakingSpace(bool nonBreakingSpace) => nonBreakingSpace ? "<nobr>" : "</nobr>";

    public static string HorizontalPosition(int percentage) => $"<pos={percentage}%>";

    public static string HorizontalSpace(int pixels) => $"<space={pixels}>";
    public static string HorizontalSpace(float em) => $"<space={em}em>";

    public static string VerticalOffset(float em) => $"<voffset={em}em>";

    public static string TextWidth(int percentage) => $"<width={percentage}%>";

    public static string Sprite(string spriteName, string? color = null)
    {
        FieldInfo? sprite = typeof(Sprites).GetFields().FirstOrDefault(x => x.Name.ToLower() == spriteName.ToLower());
        if (sprite == null)
        {
            Program.Logger.Error($"No sprite found with name {spriteName}");
            return string.Empty;
        }
    ...
    public static string Bold(bool bold) => bold ? "<b>" : "</b>";

    public static string Italic(bool italic) => italic ? "<i>" : "</i>";

    public static string Mark(bool mark, string color = "#ffff00aa") => mark ? $"<mark color=\"{color}\">" : "</mark>";

    public static string Strikethrough(bool strikethrough) => strikethrough ? "<s>" : "</s>";
